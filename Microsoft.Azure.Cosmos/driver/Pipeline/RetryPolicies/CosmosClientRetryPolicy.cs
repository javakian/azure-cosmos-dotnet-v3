...
namespace Microsoft.Azure.Cosmos
{
    ...
    /// </summary>
    internal sealed class CosmosClientRetryPolicy : ICosmosRetryPolicy
    {
        ...
        private readonly ICosmosRetryPolicy throttlingRetry;
        ...
        public CosmosClientRetryPolicy(
            GlobalEndpointManager globalEndpointManager,
            bool enableEndpointDiscovery,
            int maxRetryAttemptsOnThrottledRequests,
            int maxRetryWaitTimeInSeconds)
        {
            this.throttlingRetry = new CosmosResourceThrottleRetryPolicy(
            ...
            CancellationToken cancellationToken)
        {
            ...
            ShouldRetryResult shouldRetryResult = await this.ShouldRetryInternalAsync(
                    documentServiceResponse.StatusCode,
                    documentServiceResponse.SubStatusCode);
            if (shouldRetryResult != null)
            {
                return shouldRetryResult;
            }

            return await this.throttlingRetry.ShouldRetryAsync(documentServiceResponse, cancellationToken);
        }
        ...
        private async Task<ShouldRetryResult> ShouldRetryOnEndpointFailureAsync(bool isReadRequest, bool forceRefresh)
        {
            ...
            if (!isReadRequest)
            {
                ...
                if (this.failoverRetryCount > 1)
                {
                    ...
                    retryDelay = TimeSpan.FromMilliseconds(CosmosClientRetryPolicy.RetryIntervalInMS);
                }
            }
            else
            {
