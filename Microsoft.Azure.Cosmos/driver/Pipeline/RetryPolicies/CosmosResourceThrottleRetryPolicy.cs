...
namespace Microsoft.Azure.Cosmos
{
    ...
    internal sealed class CosmosResourceThrottleRetryPolicy : ICosmosRetryPolicy
    {
        ...
        public CosmosResourceThrottleRetryPolicy(
            int maxAttemptCount,
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (!this.IsValidThrottleStatusCode(documentServiceResponse.StatusCode))
            {
                DefaultTrace.TraceError(
                    "Operation will NOT be retried. Current attempt {0}, Status Code: {1} ",
                    this.currentAttemptCount,
            ...
            return this.ShouldRetryInternalAsync(CosmosResourceThrottleRetryPolicy.GetRetryAfter(documentServiceResponse.ResponseHeaders[HttpConstants.HttpHeaders.RetryAfterInMilliseconds]));
        }
        ...
        private static TimeSpan? GetRetryAfter(string value)
        {
